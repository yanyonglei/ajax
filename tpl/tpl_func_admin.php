...
function display($tplPath,$tplVal=null){
	...
	$tplFilePath=rtrim(TPL_ADMIN,'/').'/'.$tplPath;
...
//	$tplFilePath=rtrim(TPL_PATH,'/').'/'.$tplPath;
	...
	//开始拼接缓存文件
	$cacheFilePath=rtrim(TPL_ADMIN_CACHE,'/').'/'.str_replace('.','_',$tplPath).'.php';

	//判断文件夹
	//检测目录的权限
	if (!check_cache_dir(TPL_ADMIN_CACHE)) {
		exit('没有权限写入');
	}
...
function parseInclude($data)
{
	...
	$cacheFileName = rtrim(TPL_ADMIN_CACHE , '/') . '/' . str_replace('.' , '_' , $path) . '.php';
